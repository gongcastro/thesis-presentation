...
# make functions ---------------------------------------------------------------
generate_eli <- function(te = list(noncognate = c("gos", "pero"),
                                   cognate = c("gat", "gato")),
                         age = 0:40,
                         freq_month = 5,
                         freq_beta = 0.1,
...
words <- list("Non-cognate" = c("gos", "pero"),
              "Cognate" = c("gat", "gato"))

te_labels <- c("Non-cognate: /'gos/ (Catalan) /'pe.ro/ (Spanish)",
               "Cognate: /'gat/ (Catalan), /'ga.to/ (Spanish)")

img <- c(cat = here::here("assets", "img", "diagram-cat.png"),
...
eli_df <- bind_rows(eli_mon, eli_bil) |>
    mutate(lp = factor(lp, levels = c("Monolingual",
                                      "Bilingual")),
           ...
           id = factor(id, levels = c("Monolingual, Catalan form",
...
eli_df_summary <- eli_df |> 
    summarise(eli_sd = sd(eli),
              eli = mean(eli),
              .by = c(age, id, hypothesis, te)) |> 
    mutate(.lower = eli-eli_sd,
           .upper = eli+eli_sd)
...
eli_plot_iter <- eli_df |> 
    ...
    geom_line(data = filter(eli_df_summary,
                            ...
                            te==te_labels[1]),
              linewidth = 3/4) +
    geom_label(data = filter(aoa_df_summary,
                             hypothesis=="H0",
                             id=="Monolingual, Catalan form",
                             te==te_labels[1]),
               aes(label = round(aoa, 2),
...
aoa_plot_iter <- aoa_df |>
    ...
    geom_ribbon(data = filter(logistic_df_summary,
                ...
                linewidth = NA,
                alpha = 0.5) +
    geom_point(data = filter(aoa_df_summary,
                             id=="Monolingual, Catalan form",
                             hypothesis=="H0",
                             te==te_labels[1]),
